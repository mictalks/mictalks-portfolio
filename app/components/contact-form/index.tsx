...
export const ContactForm = () => {
    ...
    const contactFormSchema = useMemo(() => z.object({
        name: z.string().min(3, t.contact.validation.name).max(100),
        email: z.string().email(t.contact.validation.email),
        message: z.string().min(1, t.contact.validation.message).max(500),
    }), [t.contact.validation.email, t.contact.validation.message, t.contact.validation.name])
    ...
    const onSubmit = (data: ContactFormData) => {
        const subject = encodeURIComponent(`${t.contact.subject} - ${data.name}`)
        const body = encodeURIComponent(`${data.message}\n\n${t.contact.mailName}: ${data.name}\n${t.contact.mailEmail}: ${data.email}`)
                    ...
                    <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                        ...
                        <div className="flex items-center gap-3 text-xl text-gray-500">
                            ...
                            <a
                                ...
                                className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-950/60 text-gray-400 shadow-[inset_0_0_0_1px_rgba(148,163,184,0.12)] transition-all hover:-translate-y-0.5 hover:text-white"
